...
public class PlayerListener implements Listener {
			...
			if(e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK) {
				if(cooldown.containsKey(p.getUniqueId())) {
					...
					if(l+i > c) {
						e.setCancelled(true);
						double x = (l+i-c)/1000;
						p.sendMessage(ChatColor.translateAlternateColorCodes('&', plugin.getConfig().getString("cooldown-message")).replace("<time>", plugin.getFormat().format(x)));
						return;
					}
				}
	...
	public void onJoin(PlayerJoinEvent e) {
		if(plugin.isUpdateAvailable() && e.getPlayer().isOp()) {
			e.getPlayer().sendMessage(ChatColor.GRAY + "There is a new update available for " + plugin.getDescription().getName());
